...
export const MySellingHaikuList = memo(function MySellingHaikuList (props) {
  ...
  const renderToolbar = useCallback(({id}) => <>
    <button onClick={() => toggleHaikuSelling({id})}>Stop selling</button>
  </>, [toggleHaikuSelling])
  ...
  return <List items={mySellingHaikuList} renderItem={renderItem} {...props}/>;
})
